...
If you added data with ogr2ogr, the table should have a index such as `peltoalue_wkb_geometry_geom_idx`.
...
If all of your data is already properly indexed, try deleting the indexes and try recreating them.
...
To remove an existing spatial index, run:

```
DROP INDEX peltoalue_wkb_geometry_geom_idx
```

Create a new spatial index with:

```
CREATE INDEX peltoalue_geom_idx ON public.peltoalue USING GIST (wkb_geometry);
```
