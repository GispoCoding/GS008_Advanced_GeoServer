...
Chapters \@ref(rasters) and \@ref(vectors) discussed optimising raster and vector datasets for use with GeoServer to achieve better performance.
...
If WMS performance is still insufficient even after optimising the input data, the solution may be to start tiling
...
Open the URL `http://<base-url>/data` (#TODO: implement data endpoint) in a web browser to view available datasets and download a raster dataset, such as `nlsfi/ortokuvat/L4133A.tif`.
