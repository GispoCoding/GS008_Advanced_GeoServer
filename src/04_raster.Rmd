...
Raster datasets are often large in size, and poorly optimised raster data can result in significant decrease in GeoServer's performance.
Checking if raster data is correctly optimised is therefore one of the first steps that should be performed when encountering slowdowns.
...
If instead you get an "unknown command" error message or something similar, GDAL is not currently installed/available.
...
It's also very easy to use, simply open up a terminal and run:
...
```
$ gdalinfo data/nlsfi/ortokuvat/L4133A.tif
...
**However**, if you do not have any raster data available, you can use sample data from your GS instance.
Open the URL `http://<your-gs-instance>/data` (#TODO: implement data endpoint) in a web browser to view available datasets and download a raster dataset, such as `nlsfi/ortokuvat/L4133A.tif`.
...
As your GeoTIFF filesizes approach 10 GB, it's time to start thinking about splitting instead of merging.
...
By default, internal tiles are 256x256 pixels, but a different tilesize can be specified using the options `-co BLOCKXSIZE=n -co BLOCKYSIZE=n`.

Internal tiles let client software read the raster only partially.
For example, without tiles, viewing a small 100x100px subsection at the top-left corner of a 1000x1000px raster requires reading the first 100 rows of the raster completely - that's 100 rows * 1000 pixels/row = 100000 pixels.
With 256x256px tiles, only one 256x256px subtile has to be read, i.e. 65536 pixels - a decrease of ~35%.
Naturaly, the larger the raster is, the greater the difference becomes.

Try to convert some raster files to GeoTIFF format with 512x512px internal tiling.
...
For example, a 1000x1000px raster can have overviews of 500x500px, 250x250px, and 125x125px.

Tiling and overviews follow the same concept: try to minimise the amount of read data.
Monitors have a finite resolution, and rendering every pixel of a 10000x10000px raster on a 1920x1080px screen is not possible.
